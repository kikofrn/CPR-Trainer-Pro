...
- All genuine return-home actions reachable during Presenter use one guard and the existing non-modal error banner. The HeaderNav logo, Sidebar logo, ManualFlipbook close, and ManualFlipbook error reset preserve instructor state and ask the operator to stop presenting first; player close paths already stop Presenter before resetting and remain unchanged.
