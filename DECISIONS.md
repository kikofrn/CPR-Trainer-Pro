...
- W33: Corrective A makes `ManualFlipbook` the owner of each PDF.js document loading task. The component retains the established long-lived web worker and native PDF.js URL/range transport, passes the resolved `PDFDocumentProxy` directly to React-PDF pages, blocks stale state/callback work after teardown, lets child render cleanup run, waits for the owned load path to settle, and then catches `destroy()` at that owner boundary. This sequencing covers unmount, close, history restoration, manual replacement, and retry supersession without a window-level rejection filter, test allowlist, masking wait, or change to genuine load-error/retry UI. Immediate destruction was rejected because PDF.js 5.4.296 can separately throw `Worker was terminated` from its still-establishing worker path even when the returned `destroy()` promise is caught; removing the established shared worker was also rejected because it broke offline-to-online retry by requiring the worker module while offline.
- W34: Corrective B gives every mobile switch a fixed 48 px, non-shrinking track and an explicitly anchored 16 px knob. `left-1` with `translate-x-0`/`translate-x-6` produces equal 4 px outer insets in both states. One bounding-box regression finishes only the knob's CSS transition, without a timer, then proves containment and equal endpoint insets. The shared `Toggle` remains inside the lazy `MobileShell`, which App mounts only for non-Tauri mobile-web history and hides whenever the exact <=1023 px predicate is false; no >=1024 px surface can render the change.
