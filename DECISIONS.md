...
- The macOS app opens with the sidebar collapsed on every cold start. Its initial state is the literal `false`, not `useState(isTauri)`, because `isTauri` is always true in this build and would silently restore the expanded default; existing header and content-selection paths remain responsible for expanding it.
- O2 reproduced with the Instructor Manual selected: the manual nav button first rose above the fixed header and clipped the title at 985 px with the sidebar collapsed and 1178 px expanded when its sublabel wrapped. The local fix gives the manual title row a shrinkable width and truncates only the dynamic title, keeping its icons fixed; the title remains contained at both reproduction widths while the maximized layout is unchanged.
- The Phase 2.2b manual-close wiring gets a dependency-free source-level invariant in Phase 3 because this branch has no DOM test harness and adding one immediately before the conference freeze is the riskier trade. The stopgap guards both `onClose` and `onReset` against returning to the full home-reset handler; replace it with a component-level test post-conference.
...
- All three `mac-v3.0.0-rc1` attempts false-failed before compilation because the certificate preflight decoded `APPLE_CERTIFICATE` to a temporary path without the `.p12` format hint, unlike pinned tauri-macos-sign 2.3.4's `cert.p12`; Francisco proved on the reference Mac that the same p12 and password fail with `SecKeychainItemImport: Unknown format in import` at an extensionless path but import successfully with a `.p12` name. The failure therefore never proved the certificate secret bad, the original three-month-old secret's validity remains indeterminate, and the current pair was re-exported and locally validated. Existing stubbed-`security` fixtures did not independently reveal this unmodeled macOS format-detection behavior; such stubs can enforce the suffix only after a real OS execution has exposed the requirement.
