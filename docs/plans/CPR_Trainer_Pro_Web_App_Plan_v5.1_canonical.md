...
- **A — No speculative next-chapter loading at launch, amended by authorized Sheet C (2026-08-03).** The inactive player carries no `src` without explicit user intent. On web, `pointerenter`, `touchstart`, or keyboard `focus` on a specific chapter row may metadata-prefetch only that row under the §4A dedicated newest-wins prefetch epoch. Selecting another row or leaving/switching/closing the player cancels it. Adjacent/next prediction during playback remains forbidden. One media-origin preconnect per course-player surface open is permitted and is not a media request.
...
**Phase-3 semantic amendments (Claude-adjudicated, Francisco-authorized, 2026-08-03):** `playbackIntent` explicitly carries continuous-play intent through selection. Terminal selection failure is retained as a `failedRequest` record. Replay is an explicit restart command and is exempt from rule 12's same-selection no-op. Readiness is guarded by a 15-second watchdog; once playback has begun, a separate 10-second stall timer applies under the stated run conditions. Failure classification uses observable `MediaError` and browser signals and never invents an HTTP status. Reconnect always recovers to ready-paused with `resumeRequired`; audible playback resumes only after an explicit Resume command.

**Phase-3 Sheet-C latency amendment (Claude-adjudicated, Francisco-authorized, 2026-08-03):** a separate prefetch epoch may warm exactly one user-indicated row with `preload="metadata"`; it never mutates committed/pending state, never plays, and never drives UI. New row intent supersedes the old prefetch; selecting a different row cancels it; selecting the warmed row promotes the owned element without reassigning its source. Close, unmount, course/surface switch, and controller cancellation release it. The Network acceptance is zero media requests without row-level intent; hover, touch, and keyboard focus count as intent. Adjacent/next prediction remains prohibited.
...
**Phase-3 slideshow amendment (Claude-adjudicated, Francisco-authorized, 2026-08-03):** the web path uses one permanent video element across video slides. This is the single element permitted by rule 10, not a second player. Image slides remain image elements, and the Tauri keyed `AnimatePresence` path remains isolated; every unavoidable shared-JSX structural divergence is inventoried in `CONTEXT.md`.
...
9. [x] `public/404.html` — minimal branded "Page not found", self-contained, no JS (kills Pages' serve-index-for-everything default). NEW `.github/workflows/web-ci.yml` — push/PR on `web-app` only: Node 22 → `npm ci` → `npm run lint` → `npx vitest run` → install Playwright Chromium → the four-test default Playwright config → upload its externally redirected HTML report → `npm run build`. The isolated 16-row Gate-1 matrix is an explicit audit step, not routine CI. Desktop release workflow untouched.
10. [x] **Harness:** pinned `@playwright/test` devDependency + `npx playwright install chromium` in setup docs + checked-in `tests/e2e/` implementing §5.4 (readiness via `data-app-ready` + `document.fonts.ready` + locator assertions — NEVER `networkidle`; exact-match expected-error allowlists; screenshot determinism). Record Playwright version + Chromium build in `CONTEXT.md`. No screenshot images committed.
...
1. [x] **Viewport shell:** web-only class applied via `!isTauri` (NOT `@supports`-only — Tauri's WebView may also support dvh):
   ```css
...
2. [x] **Shared course-selection model** extracted from `HeaderNav` (standard/VA/pediatric/pedi+VA editions, Coming-Soon combinations, last-used-edition session memory) into a pure module/hook consumed by BOTH the desktop header (behavior unchanged) and mobile navigation. **Pure unit tests:** CPR + FA × all four edition combinations incl. Coming-Soon; video vs slideshow surface; missing/reordered course IDs; last-view behavior; desktop and mobile resolve the SAME course ID for identical inputs. Session-memory semantics preserved exactly; no new persistence.
3. [x] Navigation shell: bottom tab bar ≤1023 px (CPR & AED · First Aid · Manuals · Certs — same surfaces/icons as HeaderNav); course/chapter/edition selection in a slide-up sheet driven by the shared model, reusing Sidebar data/components restyled. **The sheet is an accessible dialog: focus containment, body-scroll locking, focus restoration on close.**
4. [x] **History state machine:** push a named sentinel (`{ehOverlay:'sheet'}` / `{ehOverlay:'player'}`) ONLY on closed→open transitions of those two UI states. Course/chapter changes create NO history entries. Opening the player while the sheet is open REPLACES the sheet sentinel. Back: player → tab/list; sheet → closed; base → leaves the site normally. UI-initiated close consumes exactly its own sentinel and never navigates away from the app. **Refresh ALWAYS normalizes to base state via `replaceState` — it never restores an overlay.** `popstate` closes overlays and never pushes (no loops). Forward restores a valid state or safely lands on the tab list. Listener removed on unmount. **Tests: direct entry, repeated Back, Forward, refresh mid-overlay (→ base), Android hardware Back.**
5. [x] Video experience: full-width 16:9; tap toggles controls; §3.4 hit regions with overlap check; landscape edge-to-edge; subtitles legible; next/prev thumb-reachable. Bottom bar hides consistently in ALL immersive states — video playback (controls hidden), manual fullscreen, slideshow fullscreen.
6. [x] A11y built in now: labels on all new icon buttons; visible focus; ARIA state for tabs/toggles; reduced-motion honored by new animations; **instructor-course cards become keyboard-operable controls (role/tabIndex/Enter+Space or real `<button>`) with unchanged visuals.**
7. [x] Acceptance per §5.5 Gate-4 row.
...
1. [x] Manuals: test and repair the EXISTING mobile flipbook path (`isMobile` = containerWidth<768 at ManualFlipbook.tsx:274; `usePortrait` at :436) on real phone widths — page-turn gestures, zoom, toolbar reachability. **Objective trigger, which decides alone: if the flipbook still fails iOS rotation/zoom testing after a HALF-DAY timebox, switch to the fallback.** The fallback is a **windowed single-page viewer: current PDF page rendered + at most one adjacent page cached, implemented with the existing react-pdf `Page` machinery (lazy/virtualized, NO new dependency, never the whole document in the DOM — iPhone memory pressure).** Decision + evidence in `DECISIONS.md`.
2. [x] Slideshows: swipe navigation with **horizontal direction-locking** (mostly-horizontal gestures drive slides; vertical scrolling and iOS edge-swipe navigation never blocked); slide videos follow §4A/Phase-3 policies; touch-simplified controls.
3. [x] Send Certs + How-To modal: single-column readable at 375 px (copy already final from Phase 3.6).
4. [x] iOS Safari pass: `playsInline` verified everywhere; `dvh` shell vs URL-bar collapse; momentum scrolling; orientation changes never lose player state.
5. [x] A11y for this phase's surfaces; desktop-only surfaces (easter eggs, mock toggles) absent from mobile UI.
...
**Gate-1 recovery governance (Francisco, Aug 2-3, 2026; recorded decision):** Codex implements web and self-verifies; Claude is the SOLE independent auditor (the implementer cannot audit its own work, so JOINT-GATE co-audit is suspended for web while Codex implements); the `web-ci.yml` default-suite Playwright job is the machine check on every push; Francisco's P9/R8 desktop smoke is the human check. JOINT-GATE resumes wherever two independent auditors exist.
...
- **Baseline/candidate:** check out the previous approved SHA and the candidate SHA in separate clean worktrees/clones, serve on separate localhost ports, run SEQUENTIALLY on the SAME machine with identical browser version, viewport, DPR, locale, timezone, color scheme, and font conditions. Never compare screenshots across OSes or machines. **Gate-1 exception (recorded decision, Aug 2026): no previously-approved SHA exists at Gate 1 - Gate 1 CREATES the first approved reference set at the candidate (two same-machine invocations x two passes each; all four hashes byte-identical). The previous-approved-SHA <=0.1% comparison protocol begins at Gate 3 against that Gate-1 reference; historical unapproved SHAs (e.g. `b550d6c`) are never visual baselines.**
...
- **Chapter-switch latency (Sheet C amendment, 2026-08-03):** metric = time from chapter activation (user gesture) to the destination's first rendered/playing frame. Measured at Gate 3 on ≥3 cold-cache chapters: one small file, one large file, and one special-character URL, five trials each. **Absolute targets on the named profile: overall median ≤ 2.5 s; worst ≤ 5.0 s for files whose `moov` atom is ≤128 KB.** Eight named heavy-`moov` files recorded in W19 use worst ≤10.0 s plus branded loading feedback visible ≤200 ms. Gate 6 re-measures with regression ≤20% vs Gate 3, re-audits the exception list, and never adds an exception silently.
