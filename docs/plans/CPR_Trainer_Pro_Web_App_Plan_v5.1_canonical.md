...
9. [x] `public/404.html` — minimal branded "Page not found", self-contained, no JS (kills Pages' serve-index-for-everything default). NEW `.github/workflows/web-ci.yml` — push/PR on `web-app` only: Node 22 → `npm ci` → `npm run lint` → `npx vitest run` → install Playwright Chromium → the four-test default Playwright config → `npm run build`. The isolated 16-row Gate-1 matrix is an explicit audit step, not routine CI. Desktop release workflow untouched.
