...
- Push 1 is the next action after this records/report-only-CSP commit and a clean worktree check. Then require web-ci and gitleaks green, validate the hosted report-only header plus representative media/manual/slideshow flows, and record hosted evidence.
- Push 2 changes only the CSP header name to enforcement, retains the directives, reruns enforcing local Pages smoke, adds hosted evidence, and pushes once. A third push remains prohibited unless an actual CSP defect appears and Francisco explicitly authorizes the corrective path.
